var videoContainer = document.getElementById('videoContainer'),
    ...
    //   vidSourcesjson = await fetch('https://tejrdev.github.io/api/video.json')
    //   vidSources = await vidSourcesjson.json();
...
let vidSourcesinfo;
async function logJSONData() {
      let vidSourcesjson = await fetch('https://tejrdev.github.io/api/video.json')
      vidSourcesinfo = await vidSourcesjson.json();
...
function fetchVideoFromNetwork(video) {
  ...
  const mp4Blob = fetch(`${video.name}`).then(response => response.blob());
